    ...
    <div className="container max-w-4xl py-10 px-6">
      ...
      <div className="mb-12">
        <motion.div
           ...
           className="flex items-center gap-2 text-cyan-600 font-bold text-xs uppercase tracking-widest mb-3"
        >
      ...
      <div className="space-y-8">
        {journeys.map((journey, i) => (
          ...
          >
            {/* Card Content */}
            <div className="glass-card rounded-[2.5rem] p-8 border border-black/[0.05] hover:border-black/10 transition-all shadow-xl relative overflow-hidden bg-white/70 group-hover:bg-white">
              
              {/* Corner Accent */}
              <div className={`absolute top-0 right-0 w-32 h-32 bg-${journey.color}-100/30 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2 group-hover:scale-150 transition-transform`} />

              <div className="flex flex-col md:flex-row gap-6">
                {/* Author Info */}
                <div className="flex md:flex-col items-center md:items-start gap-4 md:w-32">
                  <div className={`h-16 w-16 rounded-2xl bg-cyan-50 border border-cyan-100 flex items-center justify-center font-bold text-xl text-cyan-600`}>
                    {journey.avatar.substring(0, 2)}
                  </div>
                  <div className="md:mt-2">
                    <p className="font-bold text-slate-900 text-sm truncate">{journey.author}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <Briefcase className="h-3 w-3 text-slate-400" />
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{journey.company}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-2xl font-bold text-slate-900 mb-4 group-hover:text-cyan-600 transition-colors leading-tight">
                    {journey.title}
                  </h2>
                  <p className="text-slate-650 text-sm leading-relaxed whitespace-pre-line mb-6 font-medium">
                    {journey.content}
                  </p>

                  <div className="flex flex-wrap gap-2 mb-8">
                    {journey.tags.map((tag) => (
                      <Badge key={tag} className="bg-slate-100 border-slate-200 text-slate-600 hover:bg-slate-200 rounded-lg py-1 px-3 text-[10px] font-bold uppercase tracking-wider">
                        {tag}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-slate-100">
                    <div className="flex items-center gap-6">
                      <button className="flex items-center gap-2 text-slate-400 hover:text-cyan-600 transition-colors group/btn">
                        <div className="h-8 w-8 rounded-lg bg-slate-50 flex items-center justify-center group-hover/btn:bg-cyan-50 transition-colors">
                          <ThumbsUp className="h-4 w-4" />
                        </div>
                        <span className="text-xs font-bold text-slate-500">{journey.likes}</span>
                      </button>
                      <button className="flex items-center gap-2 text-slate-400 hover:text-purple-650 transition-colors group/btn">
                         <div className="h-8 w-8 rounded-lg bg-slate-50 flex items-center justify-center group-hover/btn:bg-purple-50 transition-colors">
                          <MessageCircle className="h-4 w-4" />
                        </div>
                        <span className="text-xs font-bold text-slate-500">{journey.comments}</span>
                      </button>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-all">
